...
def add_product():
    ...
    page_title = "Flowtrac - Add Product"
    form_type = "Product"
    ...
    if product:
        ...
        if request.method == "POST" and form.validate_on_submit():
            ...
            try:
                ...
                flash("Product name exists", "danger")
        page_title = "Flowtrac - Edit Product"
        form_type = "Product"
...
def view_product(name):
    ...
    product = models.Product.query.filter(sa.func.lower(models.Product.name) == sa.func.lower(name)).first()
    if product:
        page_title = "Flowtrac - {}".format(product.name)
...
def add_location():
    ...
    page_title = "Flowtrac - Add Location"
    form_type = "Location"
    return render_template("add.html", page_title=page_title, form=form, form_type=form_type)
    ...
    if location:
        ...
        if request.method == "POST" and form.validate_on_submit():
            ...
            try:
                ...
                flash("Location name exists", "danger")
        page_title = "Flowtrac - Edit Location"
        form_type = "Location"
        return render_template("edit.html", page_title=page_title, form=form, form_type=form_type)
    else:
        abort(404)
...
def view_location(name):
    ...
    if location:
        ...
        page_title = "Flowtrac - {}".format(location.name)
...
def add_movement():
    ...
    if request.method == "POST" and form.validate_on_submit():
        ...
        # When product is coming from abroad, checks are not needed on the quantity
            ...
            flash("Product moved successfully", "success")
            return redirect(url_for("movements"))
    page_title = "Flowtrac - Add Movement"
    form_type = "Movement"
    return render_template("add.html", page_title=page_title, form=form, form_type=form_type)
    ...
    if movement:
        ...
        if request.method == "POST" and form.validate_on_submit():
            ...
            # When product is coming from abroad, checks are not needed on the quantity
                ...
                flash("Movement edited successfully", "success")
                return redirect(url_for("movements"))
        page_title = "Flowtrac - Edit Movement"
        form_type = "Movement"
        return render_template("edit.html", page_title=page_title, form=form, form_type=form_type)
    else:
        abort(404)
...
def view_movement(number):
    ...
    movement = models.ProductMovement.query.filter(models.ProductMovement.id == number).first()
    if movement:
        page_title = "Flowtrac - Movement {}".format(movement.id)
